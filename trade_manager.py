...
from iron_condor_position import IronCondorPosition, IronCondorLegs, PositionStatus
from position_order_status import PositionOrderStatus
from trade_group import TradeGroup, TradeOrder
# endregion
...
class TradeManager:
    ...
    def get_similar_position_from_chain(self, position: IronCondorPosition) -> IronCondorLegs:
        ...
        short_put = self.option_chain_analyzer.get_contract_from_chain(position.get_opening_legs().get_short_put())
        short_call = self.option_chain_analyzer.get_contract_from_chain(position.get_opening_legs().get_short_call())
        long_put = self.option_chain_analyzer.get_contract_from_chain(position.get_opening_legs().get_long_put())
        long_call = self.option_chain_analyzer.get_contract_from_chain(position.get_opening_legs().get_long_call())
    ...
    def handle_close_position(self, position, closing_legs, trade_group_id, get_pnl_estimate):
    ...
    def manage_opened_position(self, position: IronCondorPosition, trade_group_id):
        ...
        if pnl["mid_percent"] >= self.config.profit_target_percent:
            ...
            return self.handle_close_position(position, closing_legs, trade_group_id, pnl)
        
        if pnl["mid_percent"] <= self.config.max_loss_percent:
            position.exit_reason = "LOSS_TARGET"
            return self.handle_close_position(position, closing_legs, trade_group_id, pnl)
        
        if self.algo.time.hour >= self.config.close_at_hour:
            position.exit_reason = "CLOSE_HOUR_TARGET"
            return self.handle_close_position(position, closing_legs, trade_group_id, pnl)
