...
from iron_condor_position import IronCondorPosition, PositionStatus
import json 
...
class PortfolioManager:
    ...
    def is_time_over_start_trading_time(self):
        target_time = datetime(self.algo.time.year, self.algo.time.month, self.algo.time.day, 
                            self.config.start_trading_hour, self.config.start_trading_minute)
        
        c_time = self.algo.time.strftime("%Y-%m-%d, %H:%M:%S")

        if self.algo.time >= target_time:
            return True
        return False

    def can_open_position(self):
        if self.is_time_over_start_trading_time(): 
            if self.trade_entered_today == True:
                return False
            else: return True
        return False
